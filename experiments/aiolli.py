...
class AiolliSimilarity(object):
    def __init__(self, data,
        ...
        self._train_set_cbf = self._data.sp_i_train_cbf
    ...
    def initialize(self):
        ...
        train2 = self._train_set_cbf.tocsc()
        cold_items_mask = np.ediff1d(train2.tocsc().indptr) == 0

        if cold_items_mask.any():
            num_cold = cold_items_mask.sum()
            percent_cold = num_cold / len(cold_items_mask) * 100
            cold_item_ids = np.where(cold_items_mask)[0]
            print("{}: Detected {} ({:.2f} %) cold items.".format(
                self.RECOMMENDER_NAME, cold_items_mask.sum(), cold_items_mask.sum() / len(cold_items_mask) * 100))
            print("Cold item IDs:", cold_item_ids)

        similarity = Compute_Similarity(train2,
                                        shrink=self.shrink,
                                        topK=self.k,
        ...
        similarity2 = Compute_Similarity(train,
                                        shrink=self.shrink,
                                        topK=self.k,
                                        normalize=self.normalize,
                                        similarity=self.similarity,
        ...
        # print(self.w_sparse.tocsc())
        # print("HEREEEEE")
        # print(similarity2.compute_similarity().tocsc())
    ...
    def get_user_recs(self, u, mask, k):
        user_id = self._data.public_users.get(np.int64(u))

        # print(self._data.public_users)
